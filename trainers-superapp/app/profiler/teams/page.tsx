...
import {
  ArrowLeft, Plus, Trash2, Loader2, ShieldAlert,
  Users, Check, X, Info
} from 'lucide-react';
...
export default function TeamsManagementPage() {
  ...
  async function fetchTeams() {
    try {
      const list = await profilerService.getTimList();
      setTeams(list);
    } catch (err) {
  ...
  async function handleAddTeam() {
    ...
    setAdding(true);
    try {
      await profilerService.addTim(newTeam.trim());
      setNewTeam('');
      await fetchTeams();
    } catch (err) {
  ...
  async function handleDeleteTeam(name: string) {
    ...
    setDeleting(name);
    try {
      await profilerService.deleteTim(name);
      await fetchTeams();
    } catch (err) {
        ...
        <div className="space-y-3">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Memuat daftar tim...</p>
            </div>
          ) : (
