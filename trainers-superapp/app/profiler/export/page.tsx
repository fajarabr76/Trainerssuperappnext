...
import {
  profilerService, Peserta, labelJabatan, labelTim,
  hitungMasaDinas, hitungUsia, formatTanggal,
  ProfilerYear, ProfilerFolder
...
function ExportContent() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    profilerService.getByBatch(selectedBatch)
      .then(setPeserta)
        ...
        <div className="bg-card rounded-3xl p-5 flex items-center justify-between shadow-sm border border-border/40">
          {loading ? (
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-medium text-foreground/60 tracking-tight">Memuat data folder...</p>
            </div>
          ) : (
            <>
              <div>
