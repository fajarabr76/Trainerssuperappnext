...
export default function QASettings() {
  ...
  // Dummy user for layout demo. Replace with real auth hook.
  ...
  const load = useCallback(async () => {
    ...
    setErrorMsg(null);
    try {
      const data = await qaService.getIndicators(activeTeam);
      setIndicators(data);
    } catch (err: any) {
  ...
  const handleAdd = async () => {
    ...
    setErrorMsg(null);
    try {
      await qaService.createIndicator(activeTeam, newName.trim(), newCategory, bobotVal, newHasNa);
      setNewName('');
      setNewBobot('10');
      setNewHasNa(false);
      setShowForm(false);
  ...
  const handleSaveEdit = async () => {
    ...
    setErrorMsg(null);
    try {
      await qaService.updateIndicator(editState.id, {
  ...
  const handleDelete = async () => {
    ...
    setErrorMsg(null);
    try {
      await qaService.deleteIndicator(confirmDelete.id);
      setConfirmDelete(null);
        ...
        <AnimatePresence mode="wait">
          {!isEditing ? (
            ...
            <motion.div
              ...
              className="px-5 py-6 bg-primary/[0.03] space-y-4"
            >
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  disabled={savingEdit}
                  className="flex-1 py-3.5 bg-primary hover:bg-primary/90 disabled:opacity-50 text-primary-foreground rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg shadow-primary/20"
                >
                  {savingEdit ? (
      ...
      <div className="flex-1 overflow-y-auto px-4 lg:px-8 py-8 space-y-6">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-xs font-bold text-foreground/30 uppercase tracking-widest">Memuat parameter...</p>
          </div>
        ) : (
