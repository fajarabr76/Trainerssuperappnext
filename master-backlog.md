...
## A. Pulihkan Pembacaan Role dari `profiles` (P1)
- [x] Audit helper auth server di `app/lib/authz.ts` agar select `profiles` hanya memakai kolom live yang benar-benar ada.
- [x] Jadikan field list profil untuk auth sebagai satu sumber kebenaran tunggal, bukan string select yang mudah drift.
- [x] Hentikan fallback senyap ke `role: ''` saat profil gagal dibaca atau tidak ditemukan.
- [x] Terapkan failure path eksplisit di auth guard dan middleware untuk kasus `profile unreadable/missing`.
- [x] Selaraskan landing/auth notice di `app/page.tsx` untuk `message=profile-unavailable`.
- [x] Rapikan type `Profile` agar sesuai schema live `public.profiles`.

## B. Verifikasi & Hardening Role (P1)
- [x] Verifikasi server-side bahwa akun `fajarabr76@gmail.com` kembali terbaca `admin`, `approved`, dan tidak terhapus.
- [x] Smoke test admin: sidebar menampilkan `Admin`, dan `/dashboard/users`, `/qa-analyzer/reports`, serta `/profiler` tetap bisa diakses.
- [x] Smoke test role salesman/lainnya untuk memastikan `leader`, `trainer`, dan `agent` tetap mendapat akses yang sesuai.
- [x] Pastikan tidak ada user valid yang lagi-lagi tampil sebagai `User` kecuali role di database memang kosong/invalid.
- [x] Jalankan `npm run lint` dan `npm run type-check` setelah perubahan.

---

## C. Design System & Refactoring

### C1. Tailwind 4 & Framer Motion Pass (P1)
...
## E. Profiler (KTP Database)

### E1. Hierarki & Folder Baru (P1)
...
### F1. Unifikasi Subpage ke Visual System Baru (P1)
- [x] `agents`
...
### K1. Skoring Jawaban dengan Gemini 3.1 Flash Lite (P0)
