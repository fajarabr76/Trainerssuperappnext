...
- Mapping riwayat Telefun sekarang menyertakan `recordingPath` and `agentRecordingPath`, sehingga UI bisa mendeteksi ketersediaan rekaman agent-only sebelum analisis.
...
### Usage & Pricing Logic Fixes

- **Improved Token Aggregation**: `parseUsageMetadata` di Railway server kini mengagregasi token dari `promptTokensDetails` jika `promptTokenCount` bernilai nol. Hal ini memastikan token input tercatat lebih akurat pada versi SDK terbaru.
- **Enforced Fallback Pricing**: `flushLiveUsage` kini menerapkan harga fallback sebesar $3.00 (input) dan $12.00 (output) per juta token untuk model "live" jika data pricing tidak ditemukan di database. Ini mencegah estimasi biaya nol yang tidak akurat saat infra database belum terupdate.
- **Structured Warnings**: Menambahkan log warning yang lebih informatif di server Railway saat fallback pricing digunakan atau saat billing tetap 0 IDR karena konfigurasi eksplisit.
