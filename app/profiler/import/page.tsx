...
import { ArrowLeft, Upload, FileSpreadsheet, CheckCircle, AlertCircle, Download, MinusCircle } from 'lucide-react';
import * as XLSX from 'xlsx';
import ExcelJS from 'exceljs';
import { profilerService, Peserta, Jabatan, DEFAULT_TIMS } from '../services/profilerService';
...
// ── Kolom template (urutan = urutan kolom di Excel) ───────────
...
function ImportExcelContent() {
  ...
  useEffect(() => {
    profilerService.getTimList().then(list => {
      if (list.length > 0) setTimList(list);
    });
  }, []);

  const downloadTemplate = async () => {
    try {
      await ExcelTemplateService.generateTemplate(batchName || 'Batch', timList);
    } catch (error) {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (!nama || nama.toLowerCase().includes('budi santoso')) continue; // Skip empty or example row
        ...
        try {
          await profilerService.create(mapped as Peserta);
            ...
            >
              {processing ? (
                <div className="space-y-3">
                  <div className="w-10 h-10 border-2 border-[#5A5A40]/20 border-t-[#5A5A40] rounded-full animate-spin mx-auto" />
