...
import { profilerService, Peserta } from '../services/profilerService';
...
export default function AddMemberPicker({ isOpen, onClose, targetBatch, onSuccess }: AddMemberPickerProps) {
  ...
  const loadPool = React.useCallback(async () => {
    setLoading(true);
    try {
      const data = await profilerService.getGlobalPesertaPool(targetBatch);
      setPool(data);
    } catch (err) {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await profilerService.copyPesertaToFolder(selectedIds, targetBatch);
      setSuccess(true);
      setTimeout(() => {
        onSuccess();
        onClose();
