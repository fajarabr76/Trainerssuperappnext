...
export default function QAPeriods() {
  ...
  // Placeholder for user and role, assuming they would be passed as props or fetched from context
  // For the purpose of making the provided snippet syntactically correct, we'll define them here.
  ...
  const loadPeriods = async () => {
    setLoading(true);
    try {
      const data = await qaService.getPeriods();
      setPeriods(data);
    } catch (err: any) {
  ...
  const handleAdd = async () => {
    ...
    setErrorMsg(null);
    try {
      const created = await qaService.createPeriod(selectedMonth, selectedYear);
  ...
  const handleDelete = async () => {
    ...
    setErrorMsg(null);
    try {
      await qaService.deletePeriod(confirmDelete.id);
  ...
  if (!mounted || loading) return (
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-[#0A0A0D] flex items-center justify-center">
