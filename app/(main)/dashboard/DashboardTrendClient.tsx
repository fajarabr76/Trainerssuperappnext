...
import { AlertCircle, TrendingDown, TrendingUp, Loader2, Activity, Target, Users } from 'lucide-react';
...
const SERVICE_COLORS: Record<string, string> = {
  call: '#3B82F6', chat: '#10B981', email: '#F59E0B', cso: '#8B5CF6',
  pencatatan: '#EC4899', bko: '#06B6D4', slik: '#F97316',
};

const SERVICE_LABELS: Record<string, string> = {
  call: 'Layanan Call', chat: 'Layanan Chat', email: 'Layanan Email', cso: 'Layanan CSO',
...
export function DashboardTrendClient({
  role,
  serviceTrendMap,
  availableYears,
  initialYear,
  ...
  if (role?.toLowerCase() !== 'trainer' && role?.toLowerCase() !== 'leader' && role?.toLowerCase() !== 'admin') {
    return null;
  }
  ...
  const handleRangeChange = async (start: number | null, end: number | null) => {
    ...
    else if (targetStart !== null && targetEnd === null) targetEnd = new Date().getMonth() + 1;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
      <div className="lg:col-span-2 rounded-3xl border border-border/40 bg-card/30 backdrop-blur-sm p-8 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="rounded-3xl border border-primary/20 bg-primary/95 text-primary-foreground p-6 flex flex-col shadow-2xl shadow-primary/10 relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
