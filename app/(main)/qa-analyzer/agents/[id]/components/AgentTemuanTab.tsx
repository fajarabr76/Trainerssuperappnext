  ...
  return (
    <div className={`relative bg-card/40 backdrop-blur-sm rounded-[3rem] border border-border/50 overflow-hidden shadow-2xl shadow-black/5 transition-opacity duration-300 ${loadingData ? 'opacity-60 pointer-events-none' : ''}`}>
      ...
      <div className="px-10 py-10 border-b border-border/50 flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-foreground/[0.01]">
        ...
        {selectedPeriod && (
          <div className="px-6 py-2.5 bg-background/50 border border-border/50 rounded-2xl text-[10px] font-black uppercase tracking-widest text-foreground/40 shadow-inner">
      ...
      <div className="divide-y divide-border/50">
        {groupedTemuan.length === 0 ? (
          <div className="px-10 py-24 text-center">
            ...
            <h4 className="text-xl font-black text-foreground/30 uppercase tracking-tighter">No issues found in this period</h4>
            <p className="text-[10px] text-foreground/20 mt-3 font-black uppercase tracking-widest leading-loose">Excellent record of service quality maintained</p>
          </div>
        ) : (
          groupedTemuan.map((group) => (
            <div key={group.no_tiket || `audit-${group.urutan}`} className="group/audit overflow-hidden">
              <div className="flex items-center justify-between px-10 py-6 bg-foreground/[0.03] border-b border-border/10 group-hover/audit:bg-foreground/[0.05] transition-colors relative">
                ...
                <div className="px-5 py-2 rounded-2xl bg-background/80 border border-border/50 text-[10px] font-black uppercase tracking-widest text-foreground/40 shadow-sm flex items-center gap-2">
                        ...
                        <div className="flex items-start justify-between gap-6 mb-6">
                          <div>
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-foreground/20">
                                Indicator Name
                              </span>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          <div className={`space-y-3 p-6 rounded-3xl bg-foreground/[0.02] border border-border/30 transition-all duration-500 ${isDeficit ? 'hover:border-amber-500/30' : 'hover:border-foreground/20'}`}>
                            <div className="flex items-center gap-2 mb-1">
                              <AlertCircle className="w-3 h-3 text-foreground/20" />
                              <p className="text-[9px] font-black uppercase tracking-[0.2em] text-foreground/20">Observation / Gap Analysis</p>
                            </div>
