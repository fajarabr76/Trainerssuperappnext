...
async def run_test():
    ...
    try:
        ...
        # Click element
        elem = frame.locator(
            "xpath=/html/body/div/div[2]/main/div/div/section/div[4]/select"
        ).nth(0)
        await asyncio.sleep(3)
        await elem.click()
        ...
        # Click element
        elem = frame.locator(
            "xpath=/html/body/div/div[2]/main/div/div/section/div[6]/button"
        ).nth(0)
        await asyncio.sleep(3)
        await elem.click()
        ...
        frame = context.pages[-1]
        assert (
            await frame.locator("xpath=//*[contains(., 'Your report is ready')]")
            .nth(0)
            .is_visible()
        ), (
            "The report should show 'Your report is ready' with an option to download after generation"
        )
        assert (
            await frame.locator("xpath=//*[contains(., 'Downloading...')]")
            .nth(0)
            .is_visible()
        ), (
            "The download should start and show 'Downloading...' after clicking the download button"
        )
