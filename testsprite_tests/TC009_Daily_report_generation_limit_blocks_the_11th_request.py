...
async def run_test():
    ...
    try:
        ...
        assert (
            await frame.locator(
                "xpath=//*[contains(., 'You have reached your daily limit.')]"
            )
            .nth(0)
            .is_visible()
        ), (
            "The user should see a daily limit message after attempting to create more than the allowed number of reports."
        )
        assert (
            await frame.locator(
                "xpath=//*[contains(., 'No download option is available for this report')]"
            )
            .nth(0)
            .is_visible()
        ), (
            "The blocked report request should show that no download option is available instead of a download state."
        )
