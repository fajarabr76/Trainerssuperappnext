-- Treat phantom-only clean sessions as valid audited presence for SIDAK summary,
-- ranking, and trend population while keeping defect metrics countable-only.
...
BEGIN
  DROP TABLE IF EXISTS temp_qa_dashboard_base;
  CREATE TEMP TABLE temp_qa_dashboard_base ON COMMIT DROP AS
  SELECT
    q.peserta_id,
    q.indicator_id,
    q.rule_indicator_id,
  ...
  WITH service_summary AS (
    SELECT
      service_type,
      COUNT(*) FILTER (WHERE is_countable_finding) AS total_defects
    FROM temp_qa_dashboard_base
...
BEGIN
  ...
  WITH param_counts AS (
    ...
    GROUP BY COALESCE(ri.name, i.name), b.period_id
  ),
  param_totals AS (
