ALTER TABLE public.qa_temuan
  ADD COLUMN IF NOT EXISTS is_phantom_padding boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS phantom_batch_id uuid;

CREATE INDEX IF NOT EXISTS idx_qa_temuan_is_phantom_padding
  ON public.qa_temuan (is_phantom_padding);
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE is_phantom_padding = false)
  ...
  WITH service_summary AS (
    SELECT
      service_type,
      COUNT(*) FILTER (WHERE is_phantom_padding = false) AS total_defects
    FROM temp_qa_range_base
  ...
  WITH agent_stats AS (
    SELECT
      ...
      b.tim,
      b.jabatan,
      COUNT(*) FILTER (WHERE b.is_phantom_padding = false) AS defects,
      BOOL_OR(b.nilai = 0 AND i.category = 'critical' AND b.is_phantom_padding = false) AS has_critical
    FROM temp_qa_range_base b
...
BEGIN
  ...
  WITH param_counts AS (
    ...
    FROM temp_qa_range_trend_base b
    JOIN qa_indicators i ON i.id = b.indicator_id
    WHERE b.is_phantom_padding = false
  ...
  period_totals AS (
    SELECT period_id, COUNT(*) FILTER (WHERE is_phantom_padding = false) AS total_count
    FROM temp_qa_range_trend_base
    GROUP BY period_id
  ),
  period_labels AS (
