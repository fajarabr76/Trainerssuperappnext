...
-- QA Performance RPCs
-- ============================================================================
-- Menjalankan agregasi berat SIDAK di database agar route dashboard dan agents
-- tidak perlu melakukan full scan `qa_temuan` di server runtime.
...
BEGIN
  ...
  WITH pareto_counts AS (
    ...
    FROM temp_qa_range_base b
    JOIN qa_indicators i ON i.id = b.indicator_id
    WHERE b.service_type = p_service_type
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'total', total_defects,
      'severity', CASE
        WHEN total_defects > 50 THEN 'Critical'
        WHEN total_defects > 30 THEN 'High'
        WHEN total_defects > 15 THEN 'Medium'
        ELSE 'Low'
      END
    )
  ), '[]'::jsonb)
...
-- ----------------------------------------------------------------------------
-- Dashboard range trend aggregate
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    SELECT COUNT(*), COUNT(DISTINCT peserta_id)
  ...
  WITH param_counts AS (
    ...
    FROM temp_qa_range_trend_base b
    JOIN qa_indicators i ON i.id = b.indicator_id
    GROUP BY i.name, b.period_id
  ),
  param_totals AS (
    SELECT name, SUM(total_count) AS grand_total
    FROM param_counts
    GROUP BY name
    ORDER BY grand_total DESC, name ASC
  ),
  period_totals AS (
    SELECT period_id, COUNT(*) AS total_count
    FROM temp_qa_range_trend_base
    GROUP BY period_id
  ),
  period_labels AS (
  ...
  SELECT jsonb_build_object(
    'labels', COALESCE((SELECT jsonb_agg(label ORDER BY month ASC) FROM period_labels), '[]'::jsonb),
    'datasets', COALESCE((
      ...
      FROM (
        SELECT jsonb_build_object(
          'label', 'Total Temuan',
          'data', (
            SELECT jsonb_agg(COALESCE(pt.total_count, 0) ORDER BY tp.month ASC)
            FROM temp_qa_range_trend_periods tp
            LEFT JOIN period_totals pt ON pt.period_id = tp.id
          ),
          'isTotal', true
        ) AS dataset
        UNION ALL
        SELECT jsonb_build_object(
          'label', p.name,
          'data', (
            SELECT jsonb_agg(COALESCE(pc.total_count, 0) ORDER BY tp.month ASC)
            FROM temp_qa_range_trend_periods tp
            LEFT JOIN param_counts pc ON pc.period_id = tp.id AND pc.name = p.name
          ),
          'isTotal', false
        ) AS dataset
        FROM param_totals p
...
-- ----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.get_qa_agent_directory_summary(
  p_year integer
)
RETURNS TABLE (
  id uuid,
...
BEGIN
  ...
  WITH eligible_agents AS (
    SELECT
      ...
      p.foto_url,
      p.jabatan
    FROM profiler_peserta p
    WHERE lower(trim(coalesce(p.tim, ''))) NOT IN ('tim om', 'tim qa', 'tim spv', 'tim da & konten')
      AND lower(trim(coalesce(p.batch_name, ''))) NOT IN ('tim om', 'tim qa', 'tim spv', 'tim da & konten')
      AND lower(trim(coalesce(p.jabatan, ''))) NOT IN (
        'qa', 'trainer', 'wfm', 'team leader', 'team_leader', 'supervisor',
        'spv', 'operational manager', 'operation_manager', 'operation manager'
  ...
  grouped_periods AS (
    ...
    FROM qa_temuan q
    JOIN qa_periods qp ON qp.id = q.period_id
    JOIN eligible_agents ea ON ea.id = q.peserta_id
    WHERE q.tahun = p_year
    GROUP BY q.peserta_id, q.period_id, qp.month, qp.year, q.service_type
  ),
  ranked_periods AS (
    SELECT
      gp.*,
      ROW_NUMBER() OVER (
        PARTITION BY gp.peserta_id
        ORDER BY gp.year DESC, gp.month DESC, gp.latest_created_at DESC, gp.service_type ASC
      ) AS global_rank,
      ROW_NUMBER() OVER (
        PARTITION BY gp.peserta_id, gp.service_type
        ORDER BY gp.year DESC, gp.month DESC, gp.latest_created_at DESC
      ) AS service_rank
    FROM grouped_periods gp
  ),
  latest_period AS (
    SELECT * FROM ranked_periods WHERE global_rank = 1
  ),
  previous_period AS (
    SELECT
      lp.peserta_id,
      COALESCE(same_service.period_id, immediate_prev.period_id) AS period_id,
      COALESCE(same_service.service_type, immediate_prev.service_type) AS service_type
    FROM latest_period lp
    LEFT JOIN ranked_periods same_service
      ON same_service.peserta_id = lp.peserta_id
     AND same_service.service_type = lp.service_type
     AND same_service.service_rank = 2
    LEFT JOIN ranked_periods immediate_prev
      ON immediate_prev.peserta_id = lp.peserta_id
     AND immediate_prev.global_rank = 2
  )
  SELECT
    ea.id,
    ea.nama,
    ea.tim,
    ea.batch_name AS batch,
    ea.batch_name,
    ea.foto_url,
    ea.jabatan,
    CASE
      WHEN lp.period_id IS NULL THEN NULL
      ELSE ROUND(qa_score_agent(ea.id, ARRAY[lp.period_id]::uuid[], lp.service_type, p_year)::numeric, 2)::float
    END AS avgScore,
    CASE
      WHEN lp.period_id IS NULL THEN 'none'
      WHEN pp.period_id IS NULL THEN 'none'
      WHEN qa_score_agent(ea.id, ARRAY[lp.period_id]::uuid[], lp.service_type, p_year)
         > qa_score_agent(ea.id, ARRAY[pp.period_id]::uuid[], pp.service_type, p_year) THEN 'up'
      WHEN qa_score_agent(ea.id, ARRAY[lp.period_id]::uuid[], lp.service_type, p_year)
         < qa_score_agent(ea.id, ARRAY[pp.period_id]::uuid[], pp.service_type, p_year) THEN 'down'
      ELSE 'same'
    END AS trend,
    CASE
      WHEN lp.period_id IS NULL OR pp.period_id IS NULL THEN NULL
      ELSE ROUND((
        qa_score_agent(ea.id, ARRAY[lp.period_id]::uuid[], lp.service_type, p_year) -
        qa_score_agent(ea.id, ARRAY[pp.period_id]::uuid[], pp.service_type, p_year)
      )::numeric, 2)::float
    END AS trendValue,
    CASE
      WHEN lp.period_id IS NULL THEN false
      ELSE qa_score_agent(ea.id, ARRAY[lp.period_id]::uuid[], lp.service_type, p_year) < 95.0
    END AS atRisk
  FROM eligible_agents ea
  LEFT JOIN latest_period lp ON lp.peserta_id = ea.id
  LEFT JOIN previous_period pp ON pp.peserta_id = ea.id
  ORDER BY ea.nama ASC;
END;
$$;
