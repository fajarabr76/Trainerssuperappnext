-- Migration: PDKT Mailbox Fanout and Shared Templates
-- Date: 2026-05-09
...
ADD COLUMN IF NOT EXISTS share_batch_id uuid DEFAULT gen_random_uuid(),
ADD COLUMN IF NOT EXISTS client_request_id text,
ADD COLUMN IF NOT EXISTS is_shared_copy boolean DEFAULT false,
ADD COLUMN IF NOT EXISTS shared_at timestamptz;

-- Idempotency index
CREATE UNIQUE INDEX IF NOT EXISTS idx_pdkt_mailbox_idempotency 
ON public.pdkt_mailbox_items (created_by_user_id, client_request_id, user_id) 
WHERE client_request_id IS NOT NULL;

-- RPC for batch insertion with fanout
...
BEGIN
    ...
    -- We use a CASE to normalize roles in SQL if needed, but here we just check common strings.
...
-- Revoke/Grant
REVOKE EXECUTE ON FUNCTION public.submit_pdkt_mailbox_batch FROM public, anon;
GRANT EXECUTE ON FUNCTION public.submit_pdkt_mailbox_batch TO authenticated;
